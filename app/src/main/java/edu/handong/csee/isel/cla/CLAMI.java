...
public class CLAMI {
	...
	public Instances labelOnInstances(Instances instances, double percentileCutoff, String positiveLabel) {
		...
		for (int instIdx = 0; instIdx < instances.numInstances(); instIdx++) {// loop size of instances times
			if (getKElement(instIdx) > cutoff)// if k[] is higher than median,
				...
				instancesCLA.instance(instIdx).setClassValue(getNegLabel(instancesCLA, positiveLabel)); // else
																										// getneglabel
		}
	...
	public Instances selectMetrices(Instances instances) {
		return instances;
	}
	
	public Instances selectInstances(Instances instances) {
		return instances;
	}
